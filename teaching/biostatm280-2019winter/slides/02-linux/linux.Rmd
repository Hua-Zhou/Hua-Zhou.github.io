...
- `locate` locates a file by name (need `mlocate` program installed):
    ```{bash}
    locate linux.Rmd
    ```
...
- `find` is similar to `locate` but has more functionalities, e.g., select files by age, size, permissions, .... , and is ubiquitous.
    ...
    ```
    ```{bash}
    find /home/hwachou -name linux.Rmd
    ```
...
- Demo script [`meanEst.R`](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/meanEst.R) implements an (terrible) estimator of mean
$$
...
- [`runSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/runSim.R) has components: (1) method implementation, (2) data generator with unspecified parameter `n`, (3) estimation based on generated data, and (4) **command argument parser**.
```{bash, echo=FALSE}
cat runSim.R
```
...
- Suppose we have 
    - [`runSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/runSim.R) which runs a simulation based on command line argument `n`.  
...
- Option 2: automate calls using R and `nohup`. [autoSim.R](http://hua-zhou.github.io/teaching/biostatm280-2019winter/slides/02-linux/autoSim.R)
