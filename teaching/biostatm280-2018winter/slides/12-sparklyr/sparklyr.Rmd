...
```
Install commonly used R packages `tidyverse`, `DBI`, `sparklyr` globally available to all users:
```{bash, eval = FALSE}
sudo R -e "install.packages(c('tidyverse', 'DBI', 'sparklyr'), repos='https://cran.rstudio.com/')"
```
