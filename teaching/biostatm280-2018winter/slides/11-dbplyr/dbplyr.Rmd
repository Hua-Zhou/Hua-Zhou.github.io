---
title: "Working with Databases - Part 2"
author: "Dr. Hua Zhou"
date: "Feb 13, 2018"
...
In this lecture we follow a [tutorial](http://www2.stat.duke.edu/~cr173/Sta523_Fa17/bigish_data.html) to handle a bigish data set. We will learn:
...
We make a smaller data set with only 14 variables:
```{r}
# magrittr for %<>%
library("magrittr")
(nyc %<>% 
...
The lubridate package facilitates handling date and time. Read [Chapter 16](http://r4ds.had.co.nz/dates-and-times.html) of _R for Data Science_ for more information.
...
Import the `nyc` data set into a SQLite database:
```{r}
library("DBI")
library("RSQLite")
...
From now on, we assume the SQLite database `NYParkingViolations.sqlite` can be accessed from current folder . It can be either the actural database file or a symbolic link to it. 
...
To force a full query and return a complete list it is necessary to use the `collect` function.
```{r}
system.time(
  nyc_sql %>%
    select(Issue_Date, Issuing_Agency, Violation_Precinct, Number, Street) %>%
