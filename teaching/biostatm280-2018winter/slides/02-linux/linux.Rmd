...
`grep` prints lines that match an expression pattern:
...
- Key authentification is more secure than password. Most passwords are weak.
...
- Script [`runSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/02-linux/runSim.R) has components: (1) method implementation, (2) command argument parser, (3) data generator with unspecified parameter `n`, and (4) estimation based on generated data.

- Call [`runSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/02-linux/runSim.R) with sample size `n=100`:
    ```{bash}
...
- `nohup` command in Linux runs program(s) immune to hangups and writes output to `nohup.out` by default. Logging out will _not_ kill the process. You can log back in to check status and results.

- `nohup` is POSIX standard thus available on both Linux and MacOS.
    ```{bash}
    nohup Rscript runSim.R n=100 &
    ```
runs `runSim.R` in background and writes output to `nohup.out`.
...
- Suppose we have 
    - R script [`runSim.R`](http://hua-zhou.github.io/teaching/biostatm280-2018winter/slides/02-linux/runSim.R) which runs a simulation based on command line argument `n`.  
