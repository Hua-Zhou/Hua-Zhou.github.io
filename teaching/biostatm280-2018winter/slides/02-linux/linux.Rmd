...
- `meanEst.R` implements a terrible estimator of mean
```{bash}
cat meanEst.R
```
...
- To run your R code non-interactively aka in batch mode:
```{bash}
...
- Specify output file:
```{bash, eval=FALSE}
# output to compResults.txt
R CMD BATCH minEffEst.R compResults.txt
```
...
- Call `R` by
```{bash, eval=FALSE}
R '--arg mu=1 sig=2 kap=3`
```
and execute
```{r, eval=FALSE}
for (arg in commandArgs(TRUE)) {
  print(paste("arg is: ", arg))
  eval(parse(text=arg))
}
print(mu)
print(sig)
