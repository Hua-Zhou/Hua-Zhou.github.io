...
```
**Surprise**! **Surprise**! Compiling into bytecode does not help at all. Following code shows that the function `testfor` is already compiled into bytecode before execution.
```{r}
sum_r
```

Let's turn off JIT (just-in-time compilation), re-define the `testfor` function, and benchmark again:
```{r}
...
Since R 3.4.0 (Apr 2017), the JIT (‘Just In Time’) byte-code compiler is enabled by default at its level 3. 
...
`Rcpp` language provides a convenient way to embed C++ code in R code.
```{r}
library(Rcpp)
...
Let's re-visit the simulation example considered in earlier lecture and [HW1 Q3](http://hua-zhou.github.io/teaching/biostatm280-2018winter/hw/hw1/hw1.html).
